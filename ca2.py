...
def test():
    ...
    print("\nTest Function\n")
    # Option 1
    bh_test = BargainHunters("Bargain Hunter", "Ethan", dict(
        {"6 Oranges": 1.00, "6 Apples": 1.10, "2 pack of Steak": 10.00, "4 salmon fillets": 7.00, "Orange Juice": 1.00,
         "4 chicken fillets": 7.50}))  # bargain hunter test class

    bh_test.name = ""
    print(bh_test)
    lc_test = LoyalCustomers("Loyal Customer", "Ethan", dict(
        {"6 Oranges": 1.00, "6 Apples": 1.10, "2 pack of Steak": 10.00, "4 salmon fillets": 7.00, "Orange Juice": 1.00,
         "4 chicken fillets": 7.50}), dict({"Apple Juice": 1.00, "Kobe Beef": 50.00, "4 salmon fillets": 7.50, "6 Organic Oranges": 2.00, "6 Organic Apples": 2.10}))  # loyal customer test class
    ...
    lc_test.print_exclusive()
    bh_test.print_normal()

    # Option 3

    loyal_cust = True  # checks if user is a loyal customer or not. False is a bargain hunter
    item1 = "Kobe Beef"
    if item1 in sc.items and item1 in lc.product_list:
        sc.items.update({item1: sc.items.get(item1) + lc.product_list.get(item1)})

    elif item1 in sc.items and item1 in lc.exclusive:
        sc.items.update({item1: sc.items.get(item1) + lc.exclusive.get(item1)})
    ...
    # this can be uncommented when an item is to be removed
    """ if len(sc.items) > 0:  # ensures that there are items to be removed in the cart
        item1 = "Kobe Beef"
        if item1 in sc.items:
            sc.items.pop(item1)
        else:
    ...
    # Option 5
    if len(sc.items) > 0:
        sc.price_of_cart = sum(sc.items.values())
        sc.add_wallet(55)   # adding 5 euro to wallet
        wallet_money = sc.get_wallet()
        if wallet_money >= sc.price_of_cart:
            wallet_money -= sc.price_of_cart
            print("You have paid, Goodbye")
            print(sc)
            # these can be uncommented
            # bh_test.bargain_notification()
            # lc_test.loyal_notification()
            exit()
        else:
            print("You do not have enough money,Goodbye")
            exit()
    else:
        print("Theres nothing in this basket")


# test()

# main will only work when test() is commented out
main()
